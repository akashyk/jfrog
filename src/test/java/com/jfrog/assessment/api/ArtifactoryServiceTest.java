...
public class ArtifactoryServiceTest {
	...
	@Test
	public void testGetTwoMostUsedArtifact() throws Exception	{
		...
		ArtifactDto artifactDto = artifactoryService.getTwoMostUsedJfrogArtifact();

		Assert.assertNotNull(artifactDto);
		Assert.assertEquals(artifactDto.getArtifactsDto().size(), 2);
		Assert.assertEquals("http:35.222.197.119.80/artifactory/jcenter-cache3", artifactDto.getArtifactsDto().get(0).getName());
		Assert.assertEquals(3, artifactDto.getArtifactsDto().get(1).getCount());
		Assert.assertEquals(4, artifactDto.getArtifactsDto().get(0).getCount());
	}
	...
	public void testShouldThrowJSONException() throws JSONException {
		when(restTemplate.postForObject(Mockito.eq("http://35.222.197.119/artifactory/api/search/aql"), Mockito.any(HttpEntity.class), Mockito.eq(String.class)))
				.thenReturn("abcd'\"");
		artifactoryService.getTwoMostUsedJfrogArtifact();
	}
